                ...
                if (commRect.width() > 0 && commRect.height() > 0) {
                    ...
                    if (safeComm.width() > 10 && safeComm.height() > 10) {
                        ...
                        try {
                            ...
                            val commResult = recognizer.process(commInputImage).kotlinx.coroutines.tasks.await()
...
content = content.replace(holeCardStr, newHoleCardStr);
...
let newCommFallbackStr = 
`                                if (inComm && foundCommCardsRaw.isEmpty()) {
                                    foundCommCardsRaw.add(Pair(card, approxCenterBoxX))
                                } else if (inHole && foundHoleCardsRaw.isEmpty()) {`;

content = content.replace(commFallbackStr, newCommFallbackStr);
